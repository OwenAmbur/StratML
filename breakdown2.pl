...
my $listfile="http://stratml.us/sitemap.xml";
...
if ($listfile=~m|^http|)
	{
	$listfilecontent=get($listfile) or die 'Unable to get listfile ' & $listfile;
	}
...
print OUT "<h1>StratML File List by Source Domains<h1>\n";
...
foreach my $key (sort keys %URL) {
	...
	if ($key!~m| |)
		...
		while($URL{$key}=~s|^([^@]*?)@||)
			...
			print OUT "<li><a href=\"".$u."\">".$uname."</a> - <a target=\"_blank\" href=\"http://legisworks.org/cgi-bin/walt5.pl?url=".$u."\">EDIT</a></li>";	
			# print $uname;
			# print ")\n";
		   	#  print "$key: $URL{$key}\n";
		   	}	   	
